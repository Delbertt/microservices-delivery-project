...
Local Setup
    ...
    git clone https://github.com/Delbertt/microservices-delivery-project.git
    cd microservices-delivery-project

    Configure environment:
    bash

    cp .env.example .env
    # Edit .env with your configurations (e.g., Kafka ports, DB credentials)

    Start infrastructure services (Kafka, databases, etc.):
    bash

    docker-compose up -d

    Build and run each microservice individually (in your IDE or via Maven):
    bash

    cd Microservices/discovery-service
    ./mvnw spring-boot:run
    # Repeat for each service (order-service, driver-service, etc.)

    Access the API Gateway at http://localhost:8080 (default).

🧪 Key Resilience Patterns Implemented
Pattern	Implementation	Purpose
Circuit Breaker	Resilience4j @CircuitBreaker	Prevents calls to failing services, fast-fails, and opens the circuit after failures.
Retry	Resilience4j @Retry	Automatically retries failed operations with configurable backoff (e.g., network timeouts).
Rate Limiter	Resilience4j @RateLimiter	Limits request rate to protect services from traffic spikes.
Bulkhead	Resilience4j @Bulkhead	Limits concurrent calls to a service to prevent resource exhaustion.
